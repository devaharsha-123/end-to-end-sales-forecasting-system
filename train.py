...
def load_excel(path: str) -> pd.DataFrame:
    ...
    logger.info(
        f"Loaded Excel: {df.shape}, columns: {df.columns.tolist()}"
    )
...
def main():
    ...
    if args.excel:
        ...
        logger.info(
            f"Loading data from Excel: {args.excel}"
        )
        ...
        logger.info(
            "No Excel file provided – using synthetic data generator."
        )
    ...
    logger.info(
        ...
        f"{df['week_start_date'].min()} "
        f"→ {df['week_start_date'].max()}"
    )
